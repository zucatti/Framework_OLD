					...
					if (r.length>0) {
						...
						if (!o[ndx]) {
							...
							for (var el in o) {
								...
								if (isDate(o[el])) {
									try {
										if (o[el].indexOf('T')>-1) values.push(qstr(o[el].split('T')[0]+' '+o[el].split('T')[0].split('Z')[0]));
									} catch(e) {
							...
							q.query(sql,function(err,rows) {
								if (rows.length==0) {
									...
									for (var el in o) {
										if (isDate(o[el])) {
											try {
												if (o[el].indexOf('T')>-1) values.push(el+'='+qstr(o[el].split('T')[0]+' '+o[el].split('T')[0].split('Z')[0]));
											} catch(e) {
												field.push(el+'='+qstr(ISODateString(o[el])));
											}											
										} else {
